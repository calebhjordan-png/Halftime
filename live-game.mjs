// live-game.mjs — STATUS ONLY (strict Game ID in column A)
import { google } from "googleapis";

const SHEET_ID = process.env.GOOGLE_SHEET_ID;
const SA_JSON  = process.env.GOOGLE_SERVICE_ACCOUNT;  // JSON string for the SA
...
async function fetchJson(url, tries=3) {
  for (let i=1;i<=tries;i++){
    try {
      const r = await fetch(url,{headers:{"User-Agent":"halftime-status/1.0"}});
...
function parseStatus(sum){
  ...
  const t = s.type || {};
  return { shortDetail: t.shortDetail||"", state: t.state||"", name:(t.name||"").toUpperCase(),
           period: Number(s.period ?? 0), displayClock: s.displayClock ?? "0:00" };
}
function getTeams(sum){
  ...
  if(Array.isArray(cps)&&cps.length>=2){
    away=cps.find(c=>c.homeAway==="away"); home=cps.find(c=>c.homeAway==="home");
  }
...
async function findRowByGameId(sheets){
  ...
  for (let i=1;i<col.length;i++){ // skip header A1
    if ((col[i][0]||"").toString().trim() === EVENT_ID) return i+1; // 1-based row number
  }
  return -1;
}
async function writeCells(sheets,rowNumber,kv){
  ...
  for(const [name,val] of Object.entries(kv)){
    ...
    updates.push({range:`${TAB_NAME}!${col}${rowNumber}`,values:[[val]]});
  }
...
(async ()=>{
  ...
  for(;;){
    ...
    if (row < 0){ console.log(`No row with Game ID ${EVENT_ID} in ${TAB_NAME} (A column).`); return; }

    const currentStatus = st.shortDetail || st.state || "unknown";
    console.log(`[${currentStatus}] ${tm.awayName} @ ${tm.homeName} Q${st.period} (${st.displayClock}) → row ${row}`);
    ...
    if (!isHalf){
      ...
      const h=colMap(hdr);
      const r=(await sheets.spreadsheets.values.get({spreadsheetId:SHEET_ID,range:`${TAB_NAME}!A${row}:Z${row}`})).data.values?.[0]||[];
      const cur=(r[h["status"]]||"").toString().trim();
      if (cur!=="Half" && cur!=="Final"){ await writeCells(sheets,row,{"status":currentStatus}); }
    } else {
      const halfScore = `${tm.awayScore}-${tm.homeScore}`;
      await writeCells(sheets,row,{"status":"Half","half score":halfScore});
