/**
 * NFL-halftime-check.mjs
 * Watches ONE NFL game (TARGET_GAME_ID). 20m cadence until Q2<10:00, then 2× minutes remaining.
 * At halftime: writes Half Score (extendable to Live Odds) once to the sheet.
 *
 * ENV (required): GOOGLE_SHEET_ID, GOOGLE_SERVICE_ACCOUNT (raw JSON), TARGET_GAME_ID
 * ENV (optional): TAB_NAME=NFL, MAX_TOTAL_MIN=200, DEBUG_MODE=true (short sleeps ~12s)
 */
import { google } from "googleapis";

const SHEET_ID = process.env.GOOGLE_SHEET_ID;
const SA_JSON  = process.env.GOOGLE_SERVICE_ACCOUNT;
const TAB_NAME = process.env.TAB_NAME || "NFL";
...
async function fetchJson(url) {
  const r = await fetch(url, { headers: { "User-Agent": "nfl-halftime-check/1.0" } });
...
function parseStatus(sum){
  ...
  const type = s.type || {};
  return {
    shortDetail: type.shortDetail || "", // "Halftime", "4:20 - 3rd", etc.
...
function decideSleep(p,clock){
  // Default 20m cadence; when Q2 and <10:00 left → 2× minutes remaining (min 1m)
  let mins=20;
  if(Number(p)===2){
...
async function writeHalftime(sheets,rowIdx,vals){
  ...
  const map={
    ...
    // Future: optional live odds (these columns exist in your sheet already)
...
(async()=>{
  ...
  let total=0;
  for(;;){
    const sum=await fetchJson(`https://site.api.espn.com/apis/site/v2/sports/football/nfl/summary?event=${EVENT_ID}`);
