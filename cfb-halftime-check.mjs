 ...
 */
import { google } from "googleapis";
import { chromium } from "playwright"; // kept for future live-odds scraping

// ---------- ENV ----------
>>>>>>> 9d9abdc (CFB: robust ESPN parsing + real matchup/status logging)
...
// ---------- ESPN parsing ----------
function parseStatus(sum){
  // Try header.competitions[0].status, then competitions[0].status
>>>>>>> 9d9abdc (CFB: robust ESPN parsing + real matchup/status logging)
...
=======
// Find teams in multiple places: header.competitions, competitions, boxscore.teams
function getTeams(sum){
  let away, home;
...
async function findRowIndex(sheets, dateET, mu){
  ...
  const hdr = rows[0].map(h=>h.toLowerCase());
  const iDate = hdr.findIndex(h=>h.startsWith("date"));   // "Date" or "Date (ET)"
  const iMu   = hdr.findIndex(h=>h==="matchup");
  if (iDate<0 || iMu<0) return -1;

  for (let i=1;i<rows.length;i++){
    const r = rows[i]||[];
    const d=(r[iDate]||"").trim(), m=(r[iMu]||"").trim();
    if (d===dateET && m===mu) return i;
  }
  return -1;
}
...
=======
  ...
  Object.entries(map).forEach(([k,v])=>{
    ...
    if (j>=0){
      const col = String.fromCharCode("A".charCodeAt(0)+j);
      const row = rowIdx + 2; // data row is 2-based
      updates.push({ range: `${TAB_NAME}!${col}${row}`, values: [[v]] });
    }
  });
  if (!updates.length) { console.log("No writable columns or nothing to write."); return 0; }
